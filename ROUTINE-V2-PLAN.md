...
- deterministic selector added in `src/lib/planning.ts`
- Today shows capacity + Must Win / Next + deadline warnings
- Week shows finite capacity + commitment progress
- local Planning manager can add/edit Workstreams, Week commitments, and dated Planned Actions
- Done / Defer / Cancel are explicit; missed actions are not auto-carried
- linked action completion uses the existing DayRecord habit snapshot path
- `npm test` — 10/10 files, **147/147 tests passed**
- `npm run build` — passed

**Phase 2 = DONE. Next: Phase 3 — Application Tracker.**
...
- dedicated local Application Tracker UI added in `src/views/applications.ts`
- add/update applications with stage, fit, URL, next action, due date, next event, fit reason, and private notes
- pipeline counts + Needs Attention + applied-this-week + live-interview metrics
- terminal `Withdrawn` is the non-destructive removal path; V1 does not hard-delete application history
- due/overdue application next actions feed the deterministic Today selector
- screening/interview/final work receives live-pipeline priority over generic top-of-funnel work
- `npm test` — 10/10 files, **148/148 tests passed**
- `npm run build` — passed

**Phase 3 = DONE. Next: Phase 4 — Replan + Weekly Review.**
...
- missed Planned Actions remain on their original date until an explicit Move / Defer / Cancel decision
- explicit Move marks the original action deferred and creates a new dated action; no silent carry-over
- Week commitment cards show done / scheduled / target blocks and flag under-scheduled commitments as AT RISK
- deadline-missed warning remains explicit and never changes the deadline automatically
- Weekly Review persists wins, misses, one bottleneck, and one next-week adjustment
- Create next week copies only Week commitments; it deliberately does **not** copy actions
- deferred actions no longer count as scheduled capacity
- `npm test` — 10/10 files, **150/150 tests passed**
- `npm run build` — passed

**Phase 4 = DONE. Next: Phase 5 — final verification + release gate.**
